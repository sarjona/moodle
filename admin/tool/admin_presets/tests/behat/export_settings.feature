...
Feature: I can add a new preset with current settings
  ...
  Scenario: Export settings with an existing name
    Given I should see "Lite"
    And I click on "Create preset" "button"
    And I set the field "Name" to "Lite"
    And I set the field "Description" to "Non-core lite preset"
    When I click on "Create preset" "button"
    Then I should see "Moodle with all of the most popular features" in the "Lite" "table_row"
    And I should see "Lite" in the "Non-core lite preset" "table_row"
