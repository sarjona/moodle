...
class helper {
    ...
    /**
     * Check whether given preset name is valid and, if it's not, return a valid one.
     * For instance, it's not possible to name presets as Lite or Full, because then they couldn't be removed.
     *
     * @param string $presetname The preset name to check.
     * @return string A valid preset name.
     */
    public static function get_valid_preset_name(string $presetname): string {
        if ($presetname == get_string('litepreset', 'tool_admin_presets') ||
                $presetname == get_string('fullpreset', 'tool_admin_presets')) {
            $presetname = get_string('renamedfrom', 'tool_admin_presets', $presetname);
        }
