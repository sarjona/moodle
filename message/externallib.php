...
class core_message_external extends external_api {
    ...
    private static function get_conversation_message_structure() {
        return new external_single_structure(
            array(
                ...
                'conversationid' => new external_value(PARAM_INT, 'The conversation id where the message belongs', VALUE_OPTIONAL),
                'text' => new external_value(PARAM_RAW, 'The text of the message'),
                'displayblocktime' => new external_value(PARAM_BOOL, 'Should we display the block time?'),
                'blocktime' => new external_value(PARAM_NOTAGS, 'The time to display above the message', VALUE_OPTIONAL),
                'timecreated' => new external_value(PARAM_INT, 'The timecreated timestamp for the message'),
                'isread' => new external_value(PARAM_INT, 'Determines if the message was read or not', VALUE_OPTIONAL),
            )
        );
    }
     ...
     * @since 3.6
     */
    public static function get_most_recent_conversation_message(int $convid, int $currentuserid) {
        global $CFG, $PAGE, $USER;
