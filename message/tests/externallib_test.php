...
require_once($CFG->dirroot . '/message/tests/fixtures/test_fake_message.php');
...
class core_message_externallib_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_conversation_messages() {
        ...
        // Send some messages back and forth.
        $time = time();
        \core_message\test\fake_message::send_fake_message_to_conversation($user1, $conversation->id, 'Yo!', $time);
        \core_message\test\fake_message::send_fake_message_to_conversation($user3, $conversation->id, 'Sup mang?', $time + 1);
        \core_message\test\fake_message::send_fake_message_to_conversation($user2, $conversation->id, 'Writing PHPUnit tests!',
            $time + 2);
        \core_message\test\fake_message::send_fake_message_to_conversation($user1, $conversation->id, 'Word.', $time + 3);
    ...
    public function test_get_conversation_messages_timefrom() {
        ...
        // Send some messages back and forth.
        $time = time();
        \core_message\test\fake_message::send_fake_message_to_conversation($user1, $conversation->id, 'Message 1', $time - 4);
        \core_message\test\fake_message::send_fake_message_to_conversation($user2, $conversation->id, 'Message 2', $time - 3);
        \core_message\test\fake_message::send_fake_message_to_conversation($user2, $conversation->id, 'Message 3', $time - 2);
        \core_message\test\fake_message::send_fake_message_to_conversation($user2, $conversation->id, 'Message 4', $time - 1);
    ...
    public function test_get_most_recent_conversation_message() {
        ...
        \core_message\test\fake_message::send_fake_message_to_conversation($user2, $conversation->id, 'Writing PHPUnit tests!',
            $time + 2);
        \core_message\test\fake_message::send_fake_message_to_conversation($user2, $conversation->id, 'Word.', $time + 3);

        // Get the most recent message.
        $result = core_message_external::get_most_recent_conversation_message($conversation->id, $user1->id);
    ...
    public function test_get_most_recent_conversation_message_as_other_user_without_cap() {
        ...
        // Ensure an exception is thrown.
        $this->expectException('moodle_exception');
        core_message_external::get_most_recent_conversation_message($conversation->id, $user2->id);
    }
    ...
    public function test_get_most_recent_conversation_message_messaging_disabled() {
        ...
        // Ensure an exception is thrown.
        $this->expectException('moodle_exception');
        core_message_external::get_most_recent_conversation_message($conversation->id, $user1->id);
    }
