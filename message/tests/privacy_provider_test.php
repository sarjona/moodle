...
class core_message_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_get_contexts_for_userid_with_messages() {
        ...
        // Check user1 has user context (for the private message) and 2 course contexts.
        $contextlist = provider::get_contexts_for_userid($user1->id);
        $contexts = $contextlist->get_contextids();
        $this->assertCount(3, $contexts);
        $this->assertTrue(in_array($user1context->id, $contexts));
        $this->assertTrue(in_array($coursecontext1->id, $contexts));
        $this->assertTrue(in_array($coursecontext2->id, $contexts));

        // Check user2 has user context (for the private message) and 1 course context.
        $contextlist = provider::get_contexts_for_userid($user2->id);
        $contexts = $contextlist->get_contextids();
        $this->assertCount(2, $contexts);
        $this->assertTrue(in_array($user2context->id, $contexts));
        $this->assertTrue(in_array($coursecontext1->id, $contexts));

        // Check user3 has has user context (because is member of a conversation) and 1 course context.
        $contextlist = provider::get_contexts_for_userid($user3->id);
        $contexts = $contextlist->get_contextids();
        $this->assertCount(2, $contexts);
        $this->assertTrue(in_array($user3context->id, $contexts));
        $this->assertTrue(in_array($coursecontext1->id, $contexts));
    ...
    public function test_export_for_context_with_group_messages() {
        ...
        // Check that user context hasn't been exported.
        $writer = writer::with_context($user1context);
        $this->assertFalse($writer->has_any_data());
