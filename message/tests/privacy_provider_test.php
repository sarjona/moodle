...
class core_message_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {
    ...
    /**
     * Test for provider::get_contexts_for_conversations().
     */
    public function test_get_contexts_for_conversations() {
        $this->resetAfterTest();
        $this->setAdminUser();
        $component = 'core_group';
        $itemtype = 'groups';
        ...
        provider::get_contexts_for_conversations($contextlist, $user1->id, $component, $itemtype);
        $this->assertCount(0, $contextlist);
        provider::get_contexts_for_conversations($contextlist, $user2->id, $component, $itemtype);
        $this->assertCount(0, $contextlist);
        ...
        $contextlist = new contextlist();
        provider::get_contexts_for_conversations($contextlist, $user3->id, $component, $itemtype);
        $this->assertCount(1, $contextlist);
        $this->assertEquals($coursecontext1, $contextlist->current());

        // Test for user4 (doesn't belong to the conversation).
        $contextlist = new contextlist();
        provider::get_contexts_for_conversations($contextlist, $user4->id, $component, $itemtype);
        $this->assertCount(0, $contextlist);
    }

    /**
     * Test for provider::get_conversations_in_context().
     */
    public function test_get_conversations_in_context() {
        $this->resetAfterTest();
        $this->setAdminUser();
        $component = 'core_group';
        $itemtype = 'groups';
        ...
        provider::get_conversations_in_context($userlist1, $component, $itemtype);
        ...
        provider::get_conversations_in_context($userlist2, $component, $itemtype);
    ...
    public function test_export_conversations() {
        ...
        provider::export_conversations($user1->id, $coursecontext1, 'core_group', 'groups');
        ...
        // Confirm the favourite conversations correct.
        $favourite = (array) $writer->get_data([
        ...
        provider::export_conversations($user2->id, $coursecontext1, 'core_group', 'groups');
    ...
    public function test_delete_conversations_for_all_users_usercontext() {
        ...
        // There should be 7 messages - 2 individual + 5 group.
        $this->assertEquals(7, $DB->count_records('messages'));
        $messages = array_keys($DB->get_records('messages'));
        $this->assertNotContains($im1, $messages);

        // There should be 1 user action - reading the group message.
        $this->assertEquals(1, $DB->count_records('message_user_actions'));
        $useractions = $DB->get_records('message_user_actions');
        $useraction = reset($useractions);
        $this->assertEquals($dbgm3->id, $useraction->messageid);

        // There should be still 4 conversations - 2 individual + 2 group.
        $this->assertEquals(4, $DB->count_records('message_conversations'));

        // There should be 8 conversation members - 1 + 2 individual + (3 + 2) group.
        $this->assertEquals(8, $DB->count_records('message_conversation_members'));

        // There should be 2 favourite conversation2 - 2 group.
