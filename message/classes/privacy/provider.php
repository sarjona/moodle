...
class provider implements
     ...
     */
    public static function get_contexts_for_conversations(contextlist $contextlist, int $userid, string $component,
    ...
    /**
     * Add the list of users who have a conversation in the specified area (component + itemtype).
     *
     ...
     * @param int $itemid Optional itemid associated with component.
     */
    public static function get_conversations_in_context(userlist $userlist, string $component, string $itemtype, int $itemid = 0) {
        $context = $userlist->get_context();
        ...
        if (!empty($component) && !empty($itemtype)) {
            ...
            if ($contextlist->current() instanceof \context_user) {
                // Only get conversations without any contextid.
                $select = "mc.contextid IS NULL";
            }
        }

        // Search all the users with any conversation in this area.
     ...
     *
     * Conversations without context (for now, the private ones) are stored in '<$context> | Messages | <Other user full name>'.
     ...
     * @param   int         $itemid Optional itemid associated with component
     */
    public static function export_conversations(int $userid, \context $context, string $component, string $itemtype,
                                                int $itemid = 0) {
        global $DB;
        ...
        if ($conversations = $DB->get_records_sql($sql, $params)) {
            ...
            foreach ($conversations as $conversation) {
                self::export_user_data_conversation_messages($userid, $conversation, $otherusers, $context);
            }
        }
    }
    ...
    public static function delete_conversations_for_all_users(\context $context, string $component, string $itemtype,
        ...
        if (!empty($component) && !empty($itemtype)) {
            ...
            // In that case, remove all the conversations for the context user.
            $contextlist = new approved_contextlist(\core_user::get_user($context->instanceid), 'core_message', [$context->id]);
            return self::delete_conversations_for_user($contextlist, $component, $itemtype, $itemid);
        }
        ...
        // Get and remove all the conversations and messages for the specified context.
    ...
    public static function delete_conversations_for_user(approved_contextlist $contextlist, string $component, string $itemtype,
        ...
        if (!empty($component) && !empty($itemtype)) {
            ...
            if ($contextlist->current() instanceof \context_user && $userid == $contextlist->current()->instanceid) {
                // Only remove conversations without any contextid.
                $select = "mc.contextid IS NULL";
            }
        }
    ...
    protected static function delete_user_data(int $userid) {
        ...
        // Create approved_contexlist for the userid.
        $usercontext = \context_user::instance($userid);
        $contextlist = new approved_contextlist(\core_user::get_user($userid), 'core_message', [$usercontext->id]);
        self::delete_conversations_for_user($contextlist, '', '');
     ...
     * @param \context $context The context to export for.
     */
    protected static function export_user_data_conversation_messages(int $userid, \stdClass $conversation,
            array $otherusers, \context $context) {
        global $DB;
        ...
        if (!empty($messagedata)) {
            ...
            if (empty($conversation->contextid)) {
                // Conversations without context are stored in 'Messages | <Other user full name>'.
