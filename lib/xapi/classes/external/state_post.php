 ...
 */
class state_post extends external_api {
    ...
    public static function execute(
        ...
        return $handler->state_save($state);
    }
    ...
    /**
     * Convert the state data jason into valir object.
     *
     * @throws xapi_exception if JSON cannot be parsed
     * @param string $statedata json encoded agent structure
     * @return stdClass the state data structure
     */
    private static function get_statedata_from_json(string $statedatajson): stdClass {
        try {
            $statedata = json_decode($statedatajson, null, 521, JSON_THROW_ON_ERROR);
        } catch (JsonException $e) {
