...
final class manager_test extends \advanced_testcase {
    ...
    /**
     * Count the number of attempts for a user in a SCORM activity.
     *
     * @covers \mod_scorm\manager::count_attempts
     */
    public function test_count_attempts(): void {
        ...
        $this->assertEquals(2, $manager->count_attempts($users['s1']->id));
        $this->assertEquals(1, $manager->count_attempts($users['s2']->id));
        $this->assertEquals(3, $manager->count_attempts());
        $manager = \mod_scorm\manager::create_from_instance($instances[1]);
        $this->assertEquals(0, $manager->count_attempts($users['s1']->id));
        $this->assertEquals(0, $manager->count_attempts($users['s2']->id));
        $this->assertEquals(0, $manager->count_attempts());
    }
