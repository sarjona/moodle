...
final class overview_test extends \advanced_testcase {
    ...
    public function test_get_extra_totalattempts_overview(
        ...
        if ($expected['totalattempts']['value'] === null) {
    ...
    public function test_get_extra_studentsattempted_overview(
        ...
        if ($expected['attempted']['value'] === null) {
            $this->assertTrue($overview->has_error());
            return;
        }
        $this->assertEquals(
    ...
    public static function get_extra_overview_items_data(): array {
        ...
        return [
            ...
            'teacher 2 - separate group' => [
                'username' => 't2',
                'activity' => 'withattempts',
                'groupmode' => SEPARATEGROUPS,
                'expected' => [
                    'attempted' => [
                        'value' => null, // Teacher 2 is not in any group, so no attempt can be counted.
                    ],
                    'totalattempts' => [
                        'value' => null, // No attempt can be counted.
                    ],
                ],
            ],
