...
class overview extends \core_courseformat\activityoverviewbase {
    ...
    public function get_actions_overview(): ?overviewitem {
        ...
        if (is_gradable(courseid: $this->course->id, itemtype: 'mod', itemmodule: 'assign', iteminstance: $this->cm->instance)) {
            $needgrading = $this->assign->count_submissions_need_grading_with_groups(array_keys($this->get_groups_for_filtering()));
            if ($needgrading > 0) {
    ...
    private function get_extra_submissions_overview(): ?overviewitem {
        ...
        if ($this->assign->get_instance()->teamsubmission) {
            // For team submissions, total represents the number of teams (instead of participants).
            if (!empty($groups)) {
                $total = count($groups);
            } else {
            ...
            $total = $this->assign->count_participants_by_groups(array_keys($groups));
        }
