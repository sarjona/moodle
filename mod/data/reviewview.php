...
$pageurl = new moodle_url('/mod/data/reviewview.php', $urlparams);
...
if ($multidelete && $canmanageentries) {
    if ($confirm) {
        ...
        $action = new moodle_url('/mod/data/reviewview.php', $submitactions);
        $cancelurl = new moodle_url('/mod/data/reviewview.php', array('d' => $data->id));
...
if ($showactivity) {
    ...
    /// Check the number of entries required before to view other participant's entries against the number of entries already made (doesn't apply to teachers)
        ...
        if ($groupmode != NOGROUPS) {
            $returnurl = new moodle_url('/mod/data/reviewview.php', ['d' => $data->id, 'mode' => $mode, 'search' => s($search),
        ...
        if (empty($records)) {
            ...
            $formurl = new moodle_url('/mod/data/reviewview.php', ['d' => $data->id, 'sesskey' => sesskey()]);
            ...
            if ($mode == 'single' && false) { // Single template
                $baseurl = '/mod/data/reviewview.php';
