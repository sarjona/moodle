...
class data_field_datetime extends data_field_base {
    ...
    function display_add_field($recordid = 0, $formdata = null) {
        ...
        if ($formdata) {
            ...
            $fieldname = 'field_' . $this->field->id . '_hour';
            $hour   = $formdata->$fieldname;
            $fieldname = 'field_' . $this->field->id . '_minute';
            $minute   = $formdata->$fieldname;
        ...
        $hourselector = html_writer::select_time(
            type: 'hours',
            name: "field_{$this->field->id}_hour",
            currenttime: $content,
            timezone: 0,
        );
        $minuteselector = html_writer::select_time(
            type: 'minutes',
            name: "field_{$this->field->id}_minute",
            currenttime: $content,
            timezone: 0,
        );

        $str .= $dayselector . $monthselector . $yearselector . $hourselector . $minuteselector;
        $str .= '</div>';
    ...
    function update_content($recordid, $value, $name='') {
        ...
        if ($this->day && $this->month && $this->year) {  // All of them have been collected now
            ...
            $calendartype = \core_calendar\type_factory::get_calendar_instance();
            $gregoriandate = $calendartype->convert_to_gregorian($this->year, $this->month, $this->day, $this->hour ?? 0, $this->minute ?? 0);
