...
final class overview_test extends \advanced_testcase {
    ...
    public function test_get_extra_deadline_overview(
        ...
        $activity = $this->getDataGenerator()->create_module(
            'workshop',
            ['course' => $course->id, 'submissionend' => $submissionend, 'assessmentend' => $assessmentend],
        );
        $cm = get_fast_modinfo($course)->get_cm($activity->cmid);
    ...
    public static function data_provider_get_extra_deadline_overview(): array {
        $submissionend = time() + 3600;
        $assessmentend = time() + 7200;
        return [
            'teacher setup phase' => [
                'user' => 'teacher',
                'currentphase' => \workshop::PHASE_SETUP,
