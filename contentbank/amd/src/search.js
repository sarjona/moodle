...
const filterContents = (body, searchTerm) => {
    const contents = Array.from(body.find(selectors.elements.cbcontent));
    const searchResults = [];
    contents.forEach((content) => {
        const contentNameElement = content.querySelector(selectors.regions.cbcontentname);
        if (searchTerm === '' || contentNameElement.innerText.toLowerCase().includes(searchTerm.toLowerCase())) {
            // The content matches the search criteria so it should be displayed and hightlighted.
            searchResults.push(content);
            contentNameElement.innerHTML = highlight(contentNameElement.innerText, searchTerm);
