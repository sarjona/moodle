...
class core_contentbank_generator extends \component_generator_base {
     ...
     * @param int $contextid The contextid where the content will be created.
    ...
    public function generate_contentbank_data(?string $contenttype, int $itemstocreate = 1, int $userid = 0,
            int $contextid = 0, bool $convert2class = true, string $filepath = 'contentfile.h5p'): array {
        global $DB, $USER;

        $contenttype = $contenttype ?? contenttype_testable\contenttype::COMPONENT;
        if (empty($contextid)) {
