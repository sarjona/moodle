...
class helper {
     ...
     * content-types exist, to avoid users without the required capability to upload malicious content. So although this method
     * returns TRUE it doesn't mean the libraries will be updated because other extra checks are done and required
     * by the Core H5P library.
     *
     * @param  stored_file $file The .h5p file to be deployed
     * @param  bool $alsouser Should only the .h5p file author be considered or the current user should also be checked?
     * @return bool Returns true if the content-type libraries can be created/updated, false otherwise.
     */
    public static function can_update_library(\stored_file $file, bool $alsouser = false): bool {
        if (is_null($file)) {
        ...
        if ($alsouser) {
            // We should also check if the current user can upgrade libraries in the context of the .h5p file.
            // This extra-check is needed sometimes, when apart from the file, current user capability has also to be checked.
