...
class player {
    ...
    private function get_assets(): array {
        ...
        $liburl = $this->factory->get_autoloader()::get_h5p_core_library_url()->out();
        $relpath = '/' . preg_replace('/^[^:]+:\/\/[^\/]+\//', '', $liburl);

        // Add core stylesheets.
        foreach (core::$styles as $style) {
            $settings['core']['styles'][] = $relpath . $style . $cachebuster;
            $this->cssrequires[] = $this->factory->get_autoloader()::get_h5p_core_library_url($style, [
                'ver' => $cachebuster,
            ]);
        }
        // Add core JavaScript.
    ...
    private function get_core_settings(): array {
        ...
        $settings = array(
            ...
            'libraryUrl' => $this->factory->get_autoloader()::get_h5p_core_library_url('js'),
    ...
    private function get_resize_code(): string {
        ...
        $template = new \stdClass();
        $template->resizeurl = $this->factory->get_autoloader()::get_h5p_core_library_url('js/h5p-resizer.js');
