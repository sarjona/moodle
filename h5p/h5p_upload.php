...
foreach (\core_h5p\framework::messages('info') as $message) {
    print $OUTPUT->notification($message, 'notifymessage');
}
foreach (\core_h5p\framework::messages('error') as $message) {
    print $OUTPUT->notification($$message->message, 'notifyproblem');
}
