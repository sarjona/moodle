...
class framework_testcase extends advanced_testcase {
    ...
    public function test_isContentSlugAvailable() {
        ...
        $slug1 = 'h5p-test-title-1';
        $slug2 = 'h5p-test-title-2';
        $slug3 = 'h5p-test-title-3';

        $library = $this->create_library_record('Library', 'Lib', 1, 2, 1);
        // Create h5p content with $slug1 ('h5p-test-title-1') as it's slug.
        $this->create_h5p_record($library->id, 'iframe', null, null, $slug1);
        // Create h5p content with $slug2 ('h5p-test-title-2') as it's slug.
        $this->create_h5p_record($library->id, 'iframe', null, null, $slug2);

        $interface = framework::instance('interface');
        // The slug 'h5p-test-title-1' should not be available (is already used).
        $isslugavailable = $interface->isContentSlugAvailable($slug1);
        $this->assertFalse($isslugavailable);
        // The slug 'h5p-test-title-2' should not be available (is already used).
        $isslugavailable = $interface->isContentSlugAvailable($slug2);
        $this->assertFalse($isslugavailable);
        // The slug 'h5p-test-title-3' should be available (is not used).
        $isslugavailable = $interface->isContentSlugAvailable($slug3);
    ...
    private function generate_h5p_data(bool $createlibraryfiles = false) : stdClass {
        // Create libraries.
        $mainlib = $this->create_library_record('MainLibrary', 'Main', 1, 0);
    ...
    private function create_h5p_record(int $mainlibid, string $embedtype = 'iframe', string $jsoncontent = null,
            string $filtered = null, string $slug = '') : int {
        global $DB;
