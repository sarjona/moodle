...
class api_testcase extends \advanced_testcase {
    ...
    public function test_delete_library(string $libraryname, int $expectedh5p, int $expectedlibraries,
        ...
        $this->assertEquals(1, $counth5p);
        $this->assertEquals(7, $counth5plibraries);
        $this->assertEquals(5, $counth5pcontents);
        $this->assertEquals(7, $counth5pdependencies);
        ...
        $this->assertEquals($expectedh5p, $counth5p);
        $this->assertEquals($expectedlibraries, $counth5plibraries);
        $this->assertEquals($expectedcontents, $counth5pcontents);
        $this->assertEquals($expecteddependencies, $counth5pdependencies);
    }
    ...
    public function test_get_dependent_libraries(string $libraryname, int $expectedvalue): void {
        ...
        if ($library) {
            $libraries = \core_h5p\api::get_dependent_libraries((int)$library->id);
        } else {
            $libraries = [];
        }
    ...
    public function test_get_library(string $libraryname, bool $emptyexpected): void {
        ...
        $library = $DB->get_record('h5p_libraries', ['machinename' => $libraryname], 'id');
        if ($library) {
            $result = \core_h5p\api::get_library((int)$library->id);
        } else {
            $result = null;
        }
