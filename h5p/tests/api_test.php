...
class api_test extends \advanced_testcase {
    ...
    public function test_get_original_content_from_pluginfile_url(): void {
        ...
        list($file, $h5p) = api::get_original_content_from_pluginfile_url($originalurl->out());
        $this->assertEquals($originalfile->get_pathnamehash(), $file->get_pathnamehash());
        $this->assertEquals($originalfile->get_contenthash(), $file->get_contenthash());
        $this->assertEquals($originalh5pid, $h5p->id);

        // Scenario 2: Referenced file (alias to originalfile).
        list($file, $h5p) = api::get_original_content_from_pluginfile_url($referencedurl->out());
        $this->assertEquals($originalfile->get_pathnamehash(), $file->get_pathnamehash());
        $this->assertEquals($originalfile->get_contenthash(), $file->get_contenthash());
        ...
        list($file, $h5p) = api::get_original_content_from_pluginfile_url($unexistingurl->out());
    ...
    public function test_can_edit_content(string $currentuser, string $fileauthor, string $filecomponent, bool $expected): void {
        ...
        if ($filecomponent === 'contentbank') {
            ...
            $contents = $generator->generate_contentbank_data('contenttype_h5p', 1, (int)$users[$fileauthor]->id, $context, true, $path);
