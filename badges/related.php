...
if (is_null($action)) {
    ...
    if ($badge->has_related()) {
        $sql = 'SELECT b.id, b.name, b.version, b.language, b.type
          FROM {badge} b
          INNER JOIN {badge_related} br ON br.relatedbadgeid = b.id
          WHERE br.badgeid = :badgeid';
        $badgerelated = $DB->get_records_sql($sql,
            array('badgeid' => $badge->id, 'courseid' => $badge->courseid));
