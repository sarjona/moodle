...
if ($backpack) {
    ...
    if ($sitebackpack->apiversion == OPEN_BADGES_V2P1) {
        ...
        // $api = new \core_badges\backpack_api2p1(externalbackpack: $sitebackpack);
        // $groups = $api->get_assertions();
        ...
        $params['selected'] = $api->get_collection_record($backpack->id);
        $form = new \core_badges\form\collections(new moodle_url('/badges/mybackpack.php'), $params);
        ...
        if ($form->is_cancelled()) {
            ...
            $api->set_backpack_collections($backpack->id, $groups);
            redirect(new moodle_url('/badges/mybadges.php'));
        }
    }
} else {
