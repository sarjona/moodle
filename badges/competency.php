...
$alignmentid = optional_param('alignmentid', null, PARAM_INT);
...
if ($mform->is_cancelled()) {
    ...
    $record = $DB->get_record_select('badge_competencies', 'badgeid = ? AND id = ?', array($badgeid, $alignmentid));
    if ($record) {
        $alignment->id = $alignmentid;
        $DB->update_record('badge_competencies', $alignment);
    } else {
        $DB->insert_record('badge_competencies', $alignment);
    }
    redirect($currenturl);
}
