...
class badge extends moodleform {
    ...
    public function definition() {
        ...
        $site = get_site();
        $issuername = $CFG->badges_defaultissuername ?: ($site->fullname ?: $site->shortname);
    ...
    public function validation($data, $files) {
        ...
        $select = 'name = :name AND status != :deleted';
        $params = ['name' => $data['name'], 'deleted' => BADGE_STATUS_ARCHIVED];
        if (array_key_exists('courseid', $data)) {
            // As there is a courseid defined, it should be used (because it's a course badge, not a site badge).
        ...
        if ($data['action'] != 'new') {
            $select .= ' AND id != :badgeid';
            $params['badgeid'] = $data['id'];
        }
        $duplicate = $DB->record_exists_select('badge', $select, $params);

        if ($duplicate) {
            if (array_key_exists('courseid', $data)) {
                $errors['name'] = get_string('error:duplicatenameincourse', 'badges');
            } else {
