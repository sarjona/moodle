...
class enable_badges extends external_api {
    ...
    public static function execute(array $badgeids): array {
        ...
        foreach ($badgeids as $badgeid) {
            ...
            // Check the badge can be enabled (so no other enabled with the same name already exists in the same context).
            $select = 'id != :badgeid AND name = :name AND (status = :active OR status = :activelocked)';
            $params = [
                'badgeid' => $badge->id,
                'name' => $badge->name,
            ...
            if ($badge->type == BADGE_TYPE_COURSE) {
                ...
                $select .= ' AND courseid IS NULL';
            }
            $duplicate = $DB->record_exists_select('badge', $select, $params);
            if ($duplicate) {
                $errormsg = ($badge->type == BADGE_TYPE_COURSE) ? 'duplicatenameincoursewhenenabling' : 'duplicatenamewhenenabling';
                \core\notification::add(
                    get_string('error:' . $errormsg, 'core_badges', $badge->name),
                    \core\notification::ERROR,
                );

                $warnings[] = [
                    'item'        => $badgeid,
                    'warningcode' => $errormsg,
                    'message'     => get_string('error:' . $errormsg, 'badges'),
                ];
                continue;
            }
