...
use core_badges\local\backpack\request\request_session;
...
class remote extends remote_base {
    ...
    public function __construct(
        ...
        $this->rendererhelper = \core\di::get(\core\output\renderer_helper::class);
    ...
    public function disconnect_backpack(): bool {
        ...
        $db->delete_records('badge_external', ['backpackid' => $this->externalbackpack->id]);
        $db->delete_records('badge_backpack', ['userid' => $USER->id]);
    ...
    private function get_postdata(
        ...
        $output = $this->rendererhelper->get_renderer('core', 'badges');
        if (!empty($exporter)) {
     ...
     * Make an API request.
     *
     * @param request_base $mapping The mapping to call the request.
    ...
    protected function request(
        ...
        if ((isset($response->status) && $response->status->success === false)) {
            // There is an error in the response.
            $url = $this->get_request_url($mapping->url, $this->externalbackpack->backpackapiurl);
            throw new \moodle_exception('URL: '. $url . ' Error: ' . implode($response->validationErrors));
        }
     ...
     */
    public function clear_system_user_session(): void {
        global $SESSION;

        $useridkey = request_session::get_token_key(BADGE_USER_ID_TOKEN);
        unset($SESSION->$useridkey);

        $expireskey = request_session::get_token_key(BADGE_EXPIRES_TOKEN);
    ...
    public function authenticate() {
        ...
        if (is_null($this->externalbackpack->email) || is_null($this->externalbackpack->password)) {
            // No email or password set, can't authenticate.
            return false;
        }

        $backpackidkey = request_session::get_token_key(BADGE_BACKPACK_ID_TOKEN);
    ...
    public function put_assertions(string $hash): array {
        ...
        if (!empty($issuedbadge->recipient->id)) {
            $assertiondata = helper::export_achievement_credential(
                OPEN_BADGES_V2,
                $hash,
            );
            ...
            if ($assertionentityid &&
                    !badges_external_get_mapping($this->externalbackpack->id, OPEN_BADGES_V2_TYPE_ASSERTION, $hash)) {
                $this->authenticate();
                ...
                $badgeadded = true;
                badges_external_create_mapping($this->externalbackpack->id, OPEN_BADGES_V2_TYPE_ASSERTION, $hash,
                    $assertionentityid);
            }
