    ...
    protected function get_context_folders(): array {
        ...
        return array_reduce($this->context->get_child_contexts(), function ($list, $childcontext) {
            if ($childcontext instanceof \context_course) {
                $coursebrowser = new contentbank_browser_context_course($childcontext);
                $hascontentbankfiles = !empty($coursebrowser->get_contentbank_files());
                if (\repository_contentbank\helper::can_access_contentbank_context($coursebrowser->context) &&
                        $hascontentbankfiles) {
