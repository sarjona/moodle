...
class repository_contentbank extends repository {
    ...
    public function get_listing($encodedpath = '', $page = '') {
        ...
        // The current context should be an instance of context_system, context_coursecat or context_course.
        ...
        if ($browser = $this->get_contentbank_browser($context)) {
     ...
     * @return \repository_contentbank\contentbankbrowser\contentbank_browser|null The content bank repository browser
     */
    private function get_contentbank_browser(\context $context):
            ?\repository_contentbank\contentbankbrowser\contentbank_browser {
        switch ($context->contextlevel) {
            case CONTEXT_SYSTEM:
                return new \repository_contentbank\contentbankbrowser\contentbank_browser_context_system($context);
            case CONTEXT_COURSECAT:
                return new \repository_contentbank\contentbankbrowser\contentbank_browser_context_coursecat($context);
            case CONTEXT_COURSE:
                return new \repository_contentbank\contentbankbrowser\contentbank_browser_context_course($context);
        }
        return null;
    }
    ...
    public function file_is_accessible($source) {
        ...
        $filename  = clean_param($fileparams->filename, PARAM_FILE);
        $contextid = clean_param($fileparams->contextid, PARAM_INT);

        $contentbankfile = $DB->get_record('contentbank_content', [
            'name' => $filename,
            'contextid' => $contextid
        ]);
        ...
        if ($contentbankfile) {
            ...
            if ($plugin && $plugin->is_enabled() && class_exists($managerclass)) {
                ...
                return $contentmanager->can_view() &&
