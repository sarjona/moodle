...
class gradingform_guide_privacy_testcase extends provider_testcase {
    ...
    public function test_export_user_preferences() {
        $this->resetAfterTest();
        $this->gradingform_guide_setup_test_scenario_data();

        // Define a user preference.
        $this->setUser($this->user1);
        set_user_preference('gradingform_guide-showmarkerdesc', 0, $this->user1);
        set_user_preference('gradingform_guide-showstudentdesc', 1, $this->user1);
    ...
    public function test_export_user_data() {
        ...
        $this->gradingform_guide_setup_test_scenario_data($defnameprefix, $now);
        $this->assertCount(2, $DB->get_records('grading_definitions'));
        ...
        $this->assertFalse($writer->has_any_data());
        $this->export_context_data_for_user($this->user1->id, $this->instancecontext1, 'gradingform_guide');
        ...
        $this->assertEquals('guide', $firstkey->method);
        ...
        $writer = writer::with_context($this->instancecontext1);
        $this->assertFalse($writer->has_any_data());
        $this->export_context_data_for_user($this->user2->id, $this->instancecontext1, 'gradingform_guide');
        $data = $writer->get_related_data([], 'gradingmethod');
        $this->assertEmpty($data);

        // Validate exported data: $assign2 - $user1 has data.
        writer::reset();
        $writer = writer::with_context($this->instancecontext2);
        $this->assertFalse($writer->has_any_data());
        $this->export_context_data_for_user($this->user1->id, $this->instancecontext2, 'gradingform_guide');
        $data = $writer->get_related_data([], 'gradingmethod');
        $this->assertCount(1, $data->definitions);
        ...
        $writer = writer::with_context($this->instancecontext2);
        $this->assertFalse($writer->has_any_data());
        $this->export_context_data_for_user($this->user2->id, $this->instancecontext2, 'gradingform_guide');
        $data = $writer->get_related_data([], 'gradingmethod');
        $this->assertCount(1, $data->definitions);
    }
     ...
     * @param timestamp $now
     */
    protected function gradingform_guide_setup_test_scenario_data($defnameprefix = null, $now = null) {
        global $DB;
